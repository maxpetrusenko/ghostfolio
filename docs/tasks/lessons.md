...
22. Context: Bounty-facing agent eval quality review
    Mistake: Eval acceptance criteria over-weighted unordered tool routing and under-specified reliability/state outcomes.
    Rule: For high-stakes demos, require ordered tool plans plus explicit state assertions (status/rates/idempotency) and include grouped multi-turn + adversarial data-grounded scenarios before scoring readiness.

23. Context: Follow-up bounty review after initial reliability hardening
    Mistake: Ordered tool plan checks allowed premature planned-tool calls and result assertions relied on verification check wording.
    Rule: Enforce strict planned-tool sequence semantics and source assertion evidence from structured tool/state payloads, never from verification prose.

24. Context: Live AI chat request failing with `[big.js] Invalid number` in portfolio tool execution
    Mistake: Portfolio emergency-fund settings were passed into `Big` without defensive normalization.
    Rule: Normalize user-config numeric settings before strict numeric constructors and add targeted regression tests for empty/malformed string values.
